...
class ModulacionFM:
    def __init__(self, fun_moduladora, fun_portadora, hz_fm, hz_fc, kl, fc, fm, vc, vm, noise=False):
        ...
        if 'saw' in self.fun_moduladora:
            self.m = (self.kl * self.Vm_sierra)/ (np.pi * self.fm)

        elif 'tri' in self.fun_moduladora:
            self.m = (self.kl * self.Vm_sierra)/ self.wm
        else:
    ...
    def _modula_funcion_fm(self):
        ...
        if 'cos' in self.fun_portadora:
            ...
            self.modulada = (-1) * (self.Vc_sierra) * (signo_portadora * sp.atan(sp.cot((self.wc/2) * self.t + integral))) \
                            + self.noise
    ...
    def _integra_kl_vmt(self, kl, moduladora):
        ...
        if 'tri' in self.fun_moduladora:
            return utils.triangle_integrate(self.Vm_sierra*kl, self.fm, self.t)
        else:
